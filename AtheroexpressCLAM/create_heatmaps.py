...
if __name__ == '__main__':
	...
	# Read Preset csv (containing info about segmentation, filtering, visualization, patching)
	...
	# Process list is a csv list containing slide_ids
	...
	for i in range(len(process_stack)):
		...
		# Raw and production save directiories for the current slide
		...
		# Region of interest (tipically not used, therefore the region of interest will be the whole slide)
	...
	#	heatmap_args.calc_heatmap = False
		...
		if heatmap_args.calc_heatmap:
			compute_from_patches(wsi_object=wsi_object, features=features, coords=coords , clam_pred=Y_hats[1], model=model, feature_extractor=feature_extractor, batch_size=exp_args.batch_size, **wsi_kwargs, 
