...
parser.add_argument('--output',metavar='output filepath + name',help='output filename',required=True)
...
if not first_run:
    model = torch.load("segmenter.pth")
if use_cuda:
    model.cuda()
model.train()
...
torch.save(model, "segmenter.pth")
