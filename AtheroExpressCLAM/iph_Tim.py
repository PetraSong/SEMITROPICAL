...
def compute_iph(case_id, filename, out_df):
    ...
    out_df = out_df.append({'case_id': case_id, 'Y_prob': res[1][0].detach().numpy() ,'IPH': (res[1][0] > 0.5).numpy(), 'area': ((attention_scores > 0).sum() / attention_scores.shape[0]).numpy()}, ignore_index=True)
    out_df.to_csv(args.csv_out)

    # if SAVE_IMAGES and res[1][0].item() > 0.5:
    if SAVE_IMAGES:
