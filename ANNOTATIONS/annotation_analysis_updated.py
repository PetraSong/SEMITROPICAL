...
def resize_image_with_imagemagick(input_path, output_path, target_width, target_height):
    cmd = ['magick',  input_path, '-resize', f'{target_width}x{target_height}!', output_path]
...
def calculate_metrics(binary_mask, predictions):
    ...
    """
    # Normalize binary_mask to have values in {0, 1}
    binary_mask_normalized = (binary_mask / 255).astype(int)

    # Flatten arrays to convert them into 1D arrays for metric calculation
    binary_mask_flat = binary_mask_normalized.flatten()
    predictions_flat = predictions.flatten()
    
    # Check if there are both classes present
    if len(np.unique(binary_mask_flat)) == 1:
        # Only one class present, cannot calculate ROC AUC
        roc_auc = float('nan')  # Not a Number (NaN)
...
for heatmap_path in glob.glob(heatmap_dir + "*_test.png"):
    ...
    # # Normalize the binary mask to have values in {0, 1} instead of {0, 255}
    # resized_binary_mask = (resized_binary_mask > 0).astype(np.uint8)  # Now binary mask has values 0 or 1
    ...
    df_results = df_results.append({
        "case_id": case_id,
        "sensitivity": enhanced_metric["sensitivity"] if not np.isnan(enhanced_metric["sensitivity"]) else "N/A",
        "specificity": enhanced_metric["specificity"] if not np.isnan(enhanced_metric["specificity"]) else "N/A",
        "ROC_AUC": enhanced_metric["ROC_AUC"] if not np.isnan(enhanced_metric["ROC_AUC"]) else "N/A",
        "optimal_threshold": enhanced_metric["optimal_threshold"] if not np.isnan(enhanced_metric["optimal_threshold"]) else "N/A",
