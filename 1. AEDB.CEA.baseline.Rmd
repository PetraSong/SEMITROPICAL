...
output:
  html_notebook:
    ...
    toc_float:
      ...
      smooth_scroll: yes
mainfont: Arial
subtitle: Accompanying 'CONVOCALS'
editor_options:
...
CONVOCALS.samplelist.withduplicates <- subset(AEDB.CEA, select = 
                                                c("STUDY_NUMBER", "UPID", "informedconsent", "Hospital", 
                                                  ...
                                                  "indexsymptoms_latest_4g", "Symptoms.Update2G", "Symptoms.Update3G") # new symptom-categories
                                              )
...
# Merge with SMA samples

```{r}
SMA_slides.raw <- fread(paste0(OUT_loc, "/20230810.study_number_only_SMA.csv"))
SMA_slides.raw[, "Slides" := "SMA"] 
names(SMA_slides.raw)[names(SMA_slides.raw) == "study_number_only"] <- "STUDY_NUMBER"
head(SMA_slides.raw)
library(dplyr)
library(stringr)
SMA_slides <- SMA_slides.raw %>% 
  mutate(across('STUDY_NUMBER', str_replace, 'AE', ''))
head(SMA_slides)
rm(SMA_slides.raw)
```
...
```{r}
table(CONVOCALS.samplelist.SMA$Slides, useNA = "ifany")
...
```{r}
fwrite(subset(CONVOCALS.samplelist.SMA, is.na(Slides)),
...
------------------------------------------------------------------------------------------------------------------------
    ...
    **Changes log**
    * v1.0.6 Added time-to-event analysis with respect to symptoms.
