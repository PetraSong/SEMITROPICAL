...
import tiling
from segmentation_utils import get_chunk
    ...
    for slide in chunk:
        ...
        if not os.path.exists(savename):
            ...
            try:
                ...
                segmentation = cv2.morphologyEx(segmentation, cv2.MORPH_CLOSE, kernel = np.ones((100, 100),np.uint8))
...
if __name__ == '__main__':
    t = time.time()
    DATA_FOLDER = '/group/glastonbury/GTEX-subset/'
    chunk = get_chunk(data_folder= DATA_FOLDER, i= int(args.index), num_tasks=int(args.num_tasks) )
    segmentation(chunk)
